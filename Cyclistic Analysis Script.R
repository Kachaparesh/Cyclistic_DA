...
current_path <- "/Users/pareshkacha/Documents/Capestone proj1/Cyclistic_DA/Cyclistic project data/"
setwd(current_path)
...
for(i in 1:length(files)){ # 
  ...
  records_without_missing_ending_trip_data <- trip_data[complete.cases(trip_data), ]
...
current_path <- "/Users/pareshkacha/Documents/Capestone proj1/Cyclistic_DA/Cyclistic project data/validated_data/"
setwd(current_path)
...
# start visualization from here%>%
  ggplot(temp_data, aes(x=start_ride_month, y=number_of_rides, color=member_casual)) +
  geom_point() +
  geom_smooth() +
